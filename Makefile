...
LIBS_WITH_C := qdrt qd qdfmt qdio qdmath qdmem qdnet qdtls qdhttp qdos qdsignal qdstr qdstrconv qdtime qdthread qdtesting qdcompress qdsqlite
...
STDLIB_MODULES := base64 bits ct flag fmt hof http io json limits math mem net tls os sb signal str strconv thread time unicode uri hex bytes crc32 sha256 regex path sort rand uuid testing compress sqlite
...
define do_build
	...
	@for lib in qdfmt qdio qdmath qdmem qdnet qdtls qdhttp qdos qdsignal qdstr qdstrconv qdtime qdthread qdtesting qdcompress qdsqlite; do \
