...
LIBS_WITH_C := qdrt qd qdfmt qdio qdmath qdmem qdnet qdtls qdhttp qdos qdsignal qdstr qdstrconv qdtime qdtesting qdcompress qdsqlite
...
STDLIB_MODULES := base64 bits ct flag fmt hof http io json limits math mem net tls os sb signal str strconv time unicode uri hex bytes crc32 sha256 regex path sort rand uuid testing compress sqlite
...
define do_build
	...
	@for lib in qdfmt qdio qdmath qdmem qdnet qdtls qdhttp qdos qdsignal qdstr qdstrconv qdtime qdtesting qdcompress qdsqlite; do \
		rm -f dist/lib/lib$$lib.a && cd $(1)/lib/$$lib && ar rcs ../../../../dist/lib/lib$$lib.a $$(ar -t lib$$lib.a) && echo "  lib$$lib.a" && cd ->/dev/null; \
	done
	@for lib in qdtls qdhttp qdcompress qdsqlite; do \
