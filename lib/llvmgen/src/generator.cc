...
class LlvmGenerator::Impl {
  public:
	...
	bool generateFunction(AstNodeFunctionDeclaration* funcNode, bool isMain, const std::string& namePrefix = "main");
...
void LlvmGenerator::Impl::setupRuntimeDeclarations() {
	...
	auto createContextFnTy = llvm::FunctionType::get(
			contextPtrTy, {builder->getInt64Ty()}, false);
	...
	auto freeContextFnTy = llvm::FunctionType::get(
			builder->getVoidTy(), {contextPtrTy}, false);
	freeContextFn = llvm::Function::Create(
			freeContextFnTy, llvm::Function::ExternalLinkage, "qd_free_context", *module);

	// qd_push_i(qd_context* ctx, int64_t value) -> qd_exec_result
	auto pushIntFnTy = llvm::FunctionType::get(
			execResultTy, {contextPtrTy, builder->getInt64Ty()}, false);
	pushIntFn = llvm::Function::Create(
			pushIntFnTy, llvm::Function::ExternalLinkage, "qd_push_i", *module);

	// qd_push_f(qd_context* ctx, double value) -> qd_exec_result
	auto pushFloatFnTy = llvm::FunctionType::get(
			execResultTy, {contextPtrTy, builder->getDoubleTy()}, false);
	pushFloatFn = llvm::Function::Create(
			pushFloatFnTy, llvm::Function::ExternalLinkage, "qd_push_f", *module);

	// qd_push_s(qd_context* ctx, const char* value) -> qd_exec_result
	auto pushStrFnTy = llvm::FunctionType::get(
			execResultTy, {contextPtrTy, llvm::PointerType::getUnqual(*context)}, false);
	pushStrFn = llvm::Function::Create(
			pushStrFnTy, llvm::Function::ExternalLinkage, "qd_push_s", *module);

	// qd_push_p(qd_context* ctx, void* value) -> qd_exec_result
	auto pushPtrFnTy = llvm::FunctionType::get(
			execResultTy, {contextPtrTy, llvm::PointerType::getUnqual(*context)}, false);
	pushPtrFn = llvm::Function::Create(
			pushPtrFnTy, llvm::Function::ExternalLinkage, "qd_push_p", *module);

	// qd_call(qd_context* ctx) -> qd_exec_result
	auto callFnTy = llvm::FunctionType::get(
			execResultTy, {contextPtrTy}, false);
	callFn = llvm::Function::Create(
			callFnTy, llvm::Function::ExternalLinkage, "qd_call", *module);

	// qd_prints(qd_context* ctx) -> qd_exec_result
	auto printsFnTy = llvm::FunctionType::get(
			execResultTy, {contextPtrTy}, false);
	printsFn = llvm::Function::Create(
			printsFnTy, llvm::Function::ExternalLinkage, "qd_prints", *module);

	// qd_nl(qd_context* ctx) -> qd_exec_result
	auto nlFnTy = llvm::FunctionType::get(
			execResultTy, {contextPtrTy}, false);
	nlFn = llvm::Function::Create(
			nlFnTy, llvm::Function::ExternalLinkage, "qd_nl", *module);
	...
	auto stackPopFnTy = llvm::FunctionType::get(
			builder->getInt32Ty(),
			{llvm::PointerType::getUnqual(*context), llvm::PointerType::getUnqual(*context)},
			false);
	stackPopFn = llvm::Function::Create(
			stackPopFnTy, llvm::Function::ExternalLinkage, "qd_stack_pop", *module);

	// qd_stack_size(qd_stack* st) -> size_t
	auto stackSizeFnTy = llvm::FunctionType::get(
			builder->getInt64Ty(),
			{llvm::PointerType::getUnqual(*context)},
			false);
	stackSizeFn = llvm::Function::Create(
			stackSizeFnTy, llvm::Function::ExternalLinkage, "qd_stack_size", *module);
}
...
void LlvmGenerator::Impl::generateInstruction(AstNodeInstruction* inst, llvm::Value* ctx) {
	...
	if (name == "prints") {
		...
		if (!runtimeFn) {
			...
			runtimeFn = llvm::Function::Create(
					fnTy, llvm::Function::ExternalLinkage, fnName, *module);
		}
...
void LlvmGenerator::Impl::generateIdentifier(AstNodeIdentifier* ident, llvm::Value* ctx, llvm::Value* forIterVar) {
	...
	if (it != userFunctions.end()) {
		...
		if (fallibleIt != fallibleFunctions.end() && fallibleIt->second) {
			...
			auto successStatus = builder->CreateSelect(hasError,
				builder->getInt64(0),
				builder->getInt64(1),
				"success_status");
...
void LlvmGenerator::Impl::generateIf(AstNodeIfStatement* ifStmt, llvm::Value* ctx, llvm::Value* forIterVar) {
	...
	llvm::BasicBlock* elseBB = ifStmt->elseBody() ?
		llvm::BasicBlock::Create(*context, "if.else", currentFn) : nullptr;
...
bool LlvmGenerator::Impl::generateFunction(AstNodeFunctionDeclaration* funcNode, bool isMain, const std::string& namePrefix) {
	...
	if (isMain) {
		...
		fn = llvm::Function::Create(
				mainFnTy, llvm::Function::ExternalLinkage, "main", *module);
		...
		std::string registerName = (namePrefix == "main") ? funcNode->name() : (namePrefix + "::" + funcNode->name());
		...
		auto result = llvm::ConstantStruct::get(
			llvm::cast<llvm::StructType>(execResultTy),
			{builder->getInt32(0)});
			...
			if (auto importNode = dynamic_cast<AstNodeImport*>(child)) {
				...
				for (const auto* func : importNode->functions()) {
					...
					if (scopedName != mangledName && !module->getFunction(scopedName)) {
						...
						auto aliasFn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage, scopedName, *module);
