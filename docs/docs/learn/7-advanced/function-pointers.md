...
```qd
fn for_each(arr:[]i64 callback:fn(i64 --) -- ) {
	0 arr len 1 for i {
...
```qd
fn filter(arr:[]i64 pred:fn(i64 -- i64) -- result:[]i64) {
	// Count matches
	0 -> count
	0 arr len 1 for i {
...
```qd
fn reduce(arr:[]i64 initial:i64 f:fn(i64 i64 -- i64) -- result:i64) {
	initial -> result
	0 arr len 1 for i {
...
fn filter(arr:[]i64 pred:Predicate -- result:[]i64) { ... }
```

## What's next?

Learn about [Anonymous Functions](anonymous-functions.md) for inline function definitions.
